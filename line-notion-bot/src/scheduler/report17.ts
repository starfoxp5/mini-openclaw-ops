...
function sumByCompany(rows: any[]) {
...
function fmt(sums: Record<string, number>) {
...
export async function build17Report(today: string) {
  const monthStart = `${today.slice(0, 8)}01`;
  const todayRows = (await queryShipmentsByDateRange(today, today)) as any[];
  const monthRows = (await queryShipmentsByDateRange(monthStart, today)) as any[];
