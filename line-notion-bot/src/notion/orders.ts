...
export async function createOrderFromParsed(parsed: ParsedEvent, groupId: string, userId: string) {
  if (!notionEnabled()) {
    return orderId();
  }
  const db = process.env.NOTION_ORDERS_DB_ID;
  if (!db) throw new Error("Missing NOTION_ORDERS_DB_ID");
...
export async function queryPendingOrdersByEta(today: string) {
  if (!notionEnabled()) {
    return [];
  }
  const db = process.env.NOTION_ORDERS_DB_ID;
