...
type TemplateKind = keyof typeof CATEGORY_MAP;

function sanitize(value: string) {
  return value.replace(/[^a-zA-Z0-9_-]+/g, "-").replace(/-+/g, "-").replace(/(^-|-$)/g, "");
}

function replaceTemplate(raw: string, vars: Record<string, string>) {
...
export async function packOrderDocuments(orderNo: string) {
  await ensureLayout();
  ...
  for (const category of categories) {
    ...
    for (const file of files) {
      if (!file.includes(`_${safeOrderNo}_`)) continue;
