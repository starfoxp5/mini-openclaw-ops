...
const STEP_SEQUENCE: BookingStep[] = [
  "parse",
  "open_page",
  "select_date",
  "select_time",
...
function withTimeout<T>(promise: Promise<T>, timeoutMs: number, step: BookingStep): Promise<T> {
...
export async function runBookingFlow(input: BookingInput, adapter: BrowserAdapter): Promise<RunResult> {
  const startedAt = now();
  ...
  let lastSuccessStep: BookingStep | "" = "";
  let transportFailureCount = 0;

  const buildSummary = async (output: BookingOutput): Promise<ExecutionSummary> => {
...
function normalizeInput(input: BookingInput): NormalizedRequest {
  const required = ["customer_name", "phone", "email", "court", "date", "time_slot"] as const;
...
function assertNormalized(value: NormalizedRequest | null): NormalizedRequest {
  if (!value) {
    throw new Error("fields are required");
  }
  return value;
...
function actionLabel(step: BookingStep): string {
  switch (step) {
    case "parse":
...
async function persistSummary(summary: ExecutionSummary) {
